...
function partOfSpeechText(word) {
  if (word?.part) {
    const abbreviations = { "及物动词": "vt.", "不及物动词": "vi.", "形容词": "adj.", "副词": "adv.", "介词": "prep.", "代词": "pron.", "连词": "conj.", "感叹词": "interj.", "数词": "num.", "冠词": "art.", "名词": "n.", "动词": "v.", "短语": "phr.", "缩写": "abbr." };
    return String(word.part).replace(/及物动词|不及物动词|形容词|副词|介词|代词|连词|感叹词|数词|冠词|名词|动词|短语|缩写/g, label => abbreviations[label]);
  }
...
function exampleFor(word) {
  const practical = window.practicalExamples?.[exampleKey(word.word)];
  const sentence = practical?.en || examples[word.word] || examples[word.word.toLowerCase()] || word.example || "";
  if (sentence && !/i\s*(?:am|'m|’m|m)\s+learning\s+the\s+word/i.test(sentence)) return sentence;
  return generatedExampleFor(word).en;
}

function exampleKey(value) {
  return String(value || "").trim().toLowerCase().replace(/[’‘]/g, "'").replace(/\s+/g, " ");
}

function exampleTranslationFor(word) {
  const practical = window.practicalExamples?.[exampleKey(word.word)];
  if (practical) return practical.zh;
  const translations = {
  ...
  return translations[word.word] || translations[word.word.toLowerCase()] || word.exampleTranslation || generatedExampleFor(word).zh;
}

function generatedExampleFor(word) {
  const term = String(word?.word || "").trim();
  const meaning = firstMeaning(word?.definition) || chineseMeaning(word?.definition) || "这个表达";
  if (!term) return { en: "", zh: "" };
  if (word?.kind === "sentence" || /[.!?]$/.test(term) || term.split(/\s+/).length >= 7) {
    return { en: /[.!?]$/.test(term) ? term : `${term}.`, zh: String(word?.definition || meaning).trim() };
  }

  const part = partOfSpeechText(word);
  if (/^to\s+/i.test(term)) {
    return { en: `It is important ${term}.`, zh: `${meaning}很重要。` };
  }
  if (part.includes("adj.")) {
    return { en: `The situation seems ${term}.`, zh: `这种情况显得${meaning.replace(/的$/, "")}。` };
  }
  if (part.includes("adv.")) {
    return { en: `She answered ${term}.`, zh: `她${meaning.endsWith("地") ? meaning : `${meaning}地`}回答。` };
  }
  if (part.includes("v.") || part.includes("vt.") || part.includes("vi.")) {
    if (/\bvi\./i.test(String(word?.definition || ""))) {
      return { en: `Things may ${term} quickly.`, zh: `事物可能很快${meaning}。` };
    }
    return { en: `They may ${term} it carefully.`, zh: `他们可能会认真地${meaning}。` };
  }
  if (part.includes("n.") || /^[A-Z]/.test(term)) {
    return { en: `${/^[A-Z]/.test(term) ? term : `The ${term}`} is important in this context.`, zh: `${meaning}在这个语境中很重要。` };
  }
  if (term.split(/\s+/).length > 1) {
    return { en: `We often use “${term}” in daily conversation.`, zh: `我们在日常对话中经常使用“${meaning}”这个表达。` };
  }
  return { en: `This example shows ${term} in a real context.`, zh: `这个例句在实际语境中展示了“${meaning}”。` };
}

let encouragementCount = 0;
let encouragementTarget = 5;
let encouragementLastAt = 0;
let encouragementTimer;

function recentLearningStats(now = Date.now()) {
  const records = words.map(word => progress[word.id]).filter(Boolean);
  const recent = records.filter(p => p.lastReviewed >= now - 7 * dayMs);
  return {
...
function resetEncouragement() {
  encouragementCount = 0;
  encouragementTarget = 5;
  encouragementLastAt = 0;
...
function maybeEncourage() {
  ...
  if (encouragementCount < encouragementTarget || Date.now() - encouragementLastAt < 60000) return;
